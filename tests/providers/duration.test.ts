...
describe('per-model durations', () => {
  it('snaps an unsupported length to the nearest one the model takes', () => {
    expect(resolveDuration('runware', 'lightricks:2@1', 5)).toBe(6);
    expect(resolveDuration('runware', 'lightricks:2@1', 9)).toBe(8);
    expect(resolveDuration('runware', 'lightricks:2@1', 30)).toBe(10);
  });

  it('keeps a supported length untouched', () => {
    expect(resolveDuration('runware', 'lightricks:2@1', 8)).toBe(8);
  ...
  it('falls back to the model default when nothing is requested', () => {
    expect(resolveDuration('runware', 'lightricks:2@1')).toBe(6);
  });

  it('only offers lengths the vendor documents', () => {
    const ltx = modelsFor('runware', 'video').find((model) => model.id === 'lightricks:2@1');
    expect(ltx?.durations).toEqual([6, 8, 10]);
