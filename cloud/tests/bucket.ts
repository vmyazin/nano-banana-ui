export function memoryBucket(config:{completeOmitsMetadata?:boolean}={}) {
  ...
  const bucket={
    ...
    async createMultipartUpload(key:string,options:{httpMetadata:{contentType:string}}){
      ...
      return {
        ...
        async complete(){const length=[...parts.values()].reduce((sum,b)=>sum+b.length,0);const bytes=new Uint8Array(length);let offset=0;for(const part of parts.values()){bytes.set(part,offset);offset+=part.length;}objects.set(key,{bytes,contentType:options.httpMetadata.contentType});const completed=result(key);if(completed&&config.completeOmitsMetadata)delete (completed as {httpMetadata?:unknown}).httpMetadata;return completed;},
