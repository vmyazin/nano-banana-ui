...
export interface Identity { subject: string; email: string; name: string }
...
export async function googleIdentity(env: Env, url: URL, state: string, verifier: string, nonce: string): Promise<Identity> {
  ...
  return { subject: claims.sub, email: claims.email, name: typeof claims.name === 'string' ? claims.name : claims.email };
}
