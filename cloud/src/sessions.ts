...
export async function createSession(env: Env, identity: Identity) {
  ...
  await env.DB.prepare('INSERT INTO account_users (id, google_subject, email, name, created_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(google_subject) DO UPDATE SET email = excluded.email, name = excluded.name')
    .bind(userId, identity.subject, identity.email, identity.name, now).run();
...
export async function currentAccount(request: Request, env: Env) {
  ...
  return env.DB.prepare('SELECT u.id, u.email, u.name FROM account_users u JOIN account_sessions s ON s.user_id = u.id WHERE s.token_hash = ? AND s.expires_at > ?')
    .bind(await hash(token), Date.now()).first<{ id: string; email: string; name: string }>();
}
