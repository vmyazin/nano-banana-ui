...
Kie image/video and fal video workspaces now use the shared cloud submission and result components. The existing key dialog includes encrypted account connections above clearly labeled browser-only keys. Browser-only execution remains an explicit choice for a signed-in user; errors and quota rejection never silently switch execution modes. Other workspace/provider paths remain pending integration.
