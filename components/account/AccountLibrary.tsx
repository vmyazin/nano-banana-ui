...
export default function AccountLibrary({localTest=false,ownerId,mode='browse',referenceLimit,onUsedReference,onAddedToTimeline,onCounts}: {
  ...
  /** Sequential and inside one busy window: "Clear" passes every id at once, and
   *  `action` would drop all but the first to its own re-entry guard. */
  ...
  return <AccountSurface label="Cloud library" className="mt-5">
    ...
    {mode==='browse'&&<div className="mt-5"><CloudJobList jobs={jobs.filter(isListedJob)} limit={10} busy={busy} onResume={id=>void action(`jobs/${id}/resume`)} onCancel={id=>void action(`jobs/${id}/cancel`)} onDismiss={id=>void action(`jobs/${id}/dismiss`)} onRemove={ids=>void removeJobs(ids)}/></div>}
    <div className="mt-5">{loading?<p role="status" className="py-6 text-center text-sm text-[var(--foreground-muted)]">Loading cloud assets…</p>:<CloudAssetGrid assets={assets} ownerId={ownerId} mode={mode} referenceLimit={referenceLimit} onUsedReference={onUsedReference} onAddedToTimeline={onAddedToTimeline} onChanged={library.refresh}/>}</div>
