...
export async function uploadAccountReferences(files:File[],signal?:AbortSignal):Promise<string[]> {
  const ids:string[]=[];
  try{
    for(const file of files){
      const upload=await accountRequest<{id:string;url:string}>('uploads',{...post({bytes:file.size,mimeType:file.type}),signal});
      ids.push(upload.id);
    ...
    await Promise.allSettled(ids.map(id=>accountRequest(`uploads/${id}`,{method:'DELETE'})));
    throw error;
  }
}
/** Caller keeps the token for retries; an uncertain response must reuse it. */
export async function submitAccountJob(token:string,request:CloudJobRequest,signal?:AbortSignal) {
  return accountRequest<{job:CloudJobView}>('jobs',{...post({token,request}),signal});
}
